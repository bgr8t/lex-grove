...
// Import the existing EmailSuite components and functionality
...
// Define preferences interfaces (reused from EmailSuite)
...
export default function EmailDraftTool() {
  const navigate = useNavigate();
  const { toast } = useToast();
  const { currentUser, loading: authLoading } = useAuth();

  // State for drafts and UI (same as EmailSuite)
...
Generate only the email content without any additional commentary or explanations.`;
  ...
  // Handle privacy preferences changes
